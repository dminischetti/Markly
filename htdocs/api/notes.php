...
switch ($method) {
    ...
    case 'POST':
        ...
        break;
    default:
        Response::json(['error' => 'unsupported_method'], 405);
}
...
function handle_post(Auth $auth, NotesRepo $notesRepo, array $payload): void
{
    ...
    if ($methodOverride === 'POST') {
        ...
        if ($title === '' || $content === '') {
            Response::json(['error' => 'validation', 'message' => 'Title and content are required.'], 422);
        }
    ...
    if ($methodOverride === 'PUT') {
        ...
        $id = (int)($payload['id'] ?? 0);
        if ($id <= 0) {
            Response::json(['error' => 'validation', 'message' => 'Missing note id.'], 422);
        }
        ...
        if ($expected <= 0) {
            Response::json(['error' => 'missing_if_match'], 428);
        }
        ...
        try {
            ...
            if ($e->getMessage() === 'version_conflict') {
                Response::json(['error' => 'version_conflict'], 409);
            }
            Response::json(['error' => 'not_found'], 404);
        }
    ...
    Response::json(['error' => 'unknown_method'], 400);
}
